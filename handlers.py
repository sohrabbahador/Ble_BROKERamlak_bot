    ...
    try:
        ...
        if chat_type == "private":
            ...
            if cb_data and txt == "check_membership":
                if await is_member(user_id): await send_msg(cid, "✅ عضویت شما تایید شد. اکنون می‌توانید از تمامی خدمات استفاده کنید.", kb_main(is_admin))
                else: await send_msg(cid, "❌ شما هنوز عضو نشده‌اید! لطفاً ابتدا عضو شوید و سپس دکمه تایید را بزنید.", None)
                return
            
            # --- سد دفاعی (بررسی اجباری بودن عضویت) ---
            if await handle_membership_flow(cid, user_id, is_admin, cb_data, txt, send_msg, MAIN_CHANNEL_URL, kb_main, is_member): return
            ...
            # --- بخش پنل مدیریت ---
            if is_admin:
                if txt == "📊 آمار ربات": await send_msg(cid, await get_bot_stats())
                elif txt == "👥 لیست کاربران": await send_msg(cid, get_users_list())
            ...
            if txt == "🔙 مرحله قبل": await handle_back_step(cid, user_id, is_admin)
                ...
                await handle_user_actions(cid, user_id, txt, s, is_admin, set_session, push_history, handle_start_flow, parse_budget_text, kb_custom_budget, kb_meter, {"search_files": search_files, "show_results": show_results, "kb_main": kb_main, "send_msg": send_msg})
