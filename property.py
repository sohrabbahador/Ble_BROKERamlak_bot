...
async def handle_user_actions(cid, user_id, txt, s, is_admin, *args, **kwargs):
    # ایمپورت‌های داخلی برای جلوگیری از Circular Import
    ...
    # 6. انتخاب تعداد خواب
        ...
        current_kb = kb_budget_2khab() if final_khab == "۲ خواب" else kb_budget_3khab() if final_khab == "۳ خواب" else kb_budget_2khab()
        await send_msg(cid, f"بودجه مورد نظر برای {final_khab} را تعیین کنید یا همه فایل‌ها را ببینید:", current_kb)
        return

    # 7. مدیریت دکمه‌های بازه بودجه
