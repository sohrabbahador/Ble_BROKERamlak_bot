...
def init_db():
    ...
    """)
    # جدول جلسات کاربر برای ذخیره فیلترها
    cur.execute("""
...
def send_pic(chat_id, photo_id, caption, kb=None):
    ...
    payload = {"chat_id": chat_id, "photo": photo_id, "caption": f"{caption}{footer}", "parse_mode": "Markdown"}
    if kb: payload["reply_markup"] = kb
    return requests.post(f"{BASE_URL}/sendPhoto", json=payload)

# ---------------------- Keyboards ----------------------
def kb_main():
    return {"keyboard": [[{"text": "🏠 خرید"}, {"text": "🔑 رهن و اجاره"}], [{"text": "🔍 جستجوی سریع"}, {"text": "⭐ علاقه‌مندی‌ها"}]], "resize_keyboard": True}

def kb_khab():
    return {"keyboard": [[{"text": "۱ خواب"}, {"text": "۲ خواب"}], [{"text": "۳ خواب"}, {"text": "۴ خواب و بیشتر"}]], "resize_keyboard": True}

def kb_budje():
    return {"keyboard": [[{"text": "۲۰ تا ۳۰ میلیارد"}, {"text": "۳۰ تا ۴۰ میلیارد"}], [{"text": "۴۰ تا ۵۰ میلیارد"}, {"text": "۵۰ میلیارد به بالا"}]], "resize_keyboard": True}

def kb_meter():
    return {"keyboard": [[{"text": "کمتر از ۱۰۰ متر"}, {"text": "۱۰۰ تا ۱۵۰ متر"}], [{"text": "۱۵۰ تا ۲۰۰ متر"}, {"text": "بیشتر از ۲۰۰ متر"}]], "resize_keyboard": True}

def kb_next():
    return {"keyboard": [[{"text": "صفحه بعد"}, {"text": "بازگشت به منو اصلی"}]], "resize_keyboard": True}
...
async def webhook(req: Request):
    ...
    # مدیریت کال‌بک دکمه‌های شیشه‌ای (علاقه‌مندی‌ها)
    if "callback_query" in data:
        cb = data["callback_query"]
        cid = cb["message"]["chat"]["id"]
        data_val = cb.get("data", "")
        if data_val.startswith("fav:"):
            fid = data_val.split(":")[1]
            conn = get_db()
            cur = conn.cursor()
            cur.execute("INSERT INTO favorites (user_id, file_id) VALUES (?, ?)", (cid, fid))
            conn.commit()
            conn.close()
    ...
    if ctype == "private":
        ...
        if txt == "/start" or txt == "بازگشت به منو اصلی":
            ...
            send_msg(cid, "سلام جناب بهادر عزیز به ربات هوشمند **BROKER Amlak** خوش آمدید.\nلطفاً نوع عملیات را انتخاب کنید:", kb_main())
            return {"ok": True}
        ...
        if any(w in txt for w in ["میلیارد", "میلیونی"]):
            b_map = {"۲۰ تا ۳۰ میلیارد": (20, 30), "۳۰ تا ۴۰ میلیارد": (30, 40), "۴۰ تا ۵۰ میلیارد": (40, 50), "۵۰ میلیارد به بالا": (50, 999)}
            vals = b_map.get(txt, (0, 999))
            set_session(user_id, budje_min=vals[0]*10**9, budje_max=vals[1]*10**9)
            send_msg(cid, "📏 متراژ مورد نظر را انتخاب کنید:", kb_meter())
            return {"ok": True}

        if "متر" in txt:
            m_map = {"کمتر از ۱۰۰ متر": (0, 100), "۱۰۰ تا ۱۵۰ متر": (100, 150), "۱۵۰ تا ۲۰۰ متر": (150, 200), "بیشتر از ۲۰۰ متر": (200, 999)}
            vals = m_map.get(txt, (0, 999))
            set_session(user_id, meter_min=vals[0], meter_max=vals[1])
            ...
            if not results:
                ...
                send_msg(cid, "📄 برای دیدن موارد بیشتر:", kb_next())
            return {"ok": True}
